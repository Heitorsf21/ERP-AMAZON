...
describe("sp optimizer report parsers", () => {
  ...
  it("keeps the same natural key when only portfolio or advertised product metadata changes", () => {
    const baseRow: AdsReportRow = {
      date: "2026-05-30",
      campaignId: "camp-1",
      adGroupId: "ag-1",
    ...
    const [withPortfolio, withoutPortfolio] = parseSpTargetingRows([
      {
        ...baseRow,
        portfolioId: "portfolio-1",
        advertisedSku: "SKU-1",
        advertisedAsin: "ASIN-1",
      },
      baseRow,
    ]);

    expect(withPortfolio?.naturalKey).toBe(withoutPortfolio?.naturalKey);
  });
