...
- Agenda: Tarefa (empresa/pessoal, responsável único; recorrentes materializadas a partir de TarefaRecorrente) · TarefaRecorrente (molde DIARIA/SEMANAL/MENSAL/PERSONALIZADA) · ContaFixa (recorrente/não recorrente). Ocorrências de conta fixa vivem em `ContaPagar` (`contaFixaId` + `competencia` "YYYY-MM", com `@@unique([contaFixaId, competencia])`).
...
Área central de organização. Módulos: `src/modules/tarefas/` (CRUD + visibilidade + recorrência), `src/modules/contas-fixas/` (recorrência pura + ocorrências idempotentes), `src/modules/agenda/` (agregação por período). UI: `src/components/agenda/` — `agenda-view` com **calendário central** (toggle Dia/Semana/Mês, abre em **Semana**) + **painel "A concluir" fixo** (buckets Atrasadas/Hoje/Esta semana/Sem prazo em `painel-a-concluir`); `item-agenda` (item compartilhado), `dialog-tarefa`, `dialog-tarefa-recorrente`, `dialog-contas-fixas`. Item na sidebar (grupo Financeiro).
- **Tarefas**: título, descrição, prazo (opcional → backlog), status ABERTA/CONCLUIDA/CANCELADA, visibilidade EMPRESA/PESSOAL, responsável único. **Segurança (anti-IDOR)**: PESSOAL só é visível/editável pelo dono — aplicado no SERVER (`whereTarefasVisiveis`/`podeVerTarefa` em `tarefas/visibilidade.ts`, e `buscarVisivel` no repo). Criar/editar PESSOAL força `responsavelId = session.uid`. Endpoints `/api/tarefas` (POST), `/api/tarefas/[id]` (PATCH/DELETE), `/api/tarefas/[id]/concluir` (POST) — `requireSession`. Lookup nunca revela existência de tarefa alheia (404). **Recorrentes**: molde `TarefaRecorrente` (DIARIA/SEMANAL/MENSAL/PERSONALIZADA + término NUNCA/DATA/N_VEZES); motor puro `tarefas/recorrencia.ts` (`planejarOcorrenciasTarefas`, chave idempotente `yyyy-MM-dd` em `Tarefa.chaveOcorrencia`, `@@unique([tarefaRecorrenteId, chaveOcorrencia])`); `garantirOcorrenciasTarefas({de,ate})` materializa `Tarefa` reais no GET da agenda (clamp 400d, P2002-safe, espelha contas fixas); editar molde com `aplicarFuturas` propaga às futuras em aberto (nunca toca concluídas). Endpoints `/api/tarefas-recorrentes` (GET/POST), `/[id]` (PATCH/DELETE=desativar) — `requireSession`, PESSOAL força dono.
- **Contas fixas**: descrição, valor (centavos), `diaVencimento` (1..31, clampa ao último dia do mês — ex: 31 em fev → 28/29), recorrente/não, categoria/fornecedor opcionais, ativa. Recorrência pura em `contas-fixas/recorrencia.ts` (`planejarOcorrencias`, `vencimentoDaCompetencia` ao meio-dia UTC). `garantirOcorrencias({de,ate})` materializa em `ContaPagar` de forma **idempotente** (set de existentes + `@@unique`), **clampando o intervalo a 400 dias** (anti-DoS, pois roda em GET da agenda). Não recorrente → 1 ocorrência na competência escolhida (`ContaFixa.competenciaUnica`; UI usa date picker de data completa, fallback = mês de criação para registros antigos). Categoria/fornecedor ausentes usam sentinela "Contas Fixas". Editar com `sincronizarFuturas: true` propaga valor/vencimento/descrição às ocorrências **futuras em aberto** (`sincronizarOcorrenciasFuturas`: atualiza competências ainda planejadas, soft-delete + zera `competencia` nas que saíram do plano; **nunca toca em PAGA**). **Meses ANTERIORES ao atual nascem PAGOS com saída de caixa** (`ocorrenciaDeveVirPaga` compara competência YYYY-MM; `criarOcorrenciaPaga` cria `ContaPagar` PAGA + `Movimentacao` SAIDA com `dataCaixa=vencimento`, em transação) — mês atual/futuro nascem ABERTOS. `garantirOcorrencias` retorna `{criadas, pagas}`. Contas a Pagar (GET `/api/contas`) materializa o **mês atual** ao listar (sem caixa retroativo). Pagar ocorrência: botão na Agenda (dialog de data → `/api/contas/[id]/pagar`) ou na lista de Contas a Pagar. Backfill retroativo via botão "Gerar histórico" no `<DialogContasFixas>` (POST `/api/contas-fixas/ocorrencias` {de, ate}). Endpoints `/api/contas-fixas` (GET/POST), `/api/contas-fixas/[id]` (PATCH/DELETE=desativar), `/api/contas-fixas/ocorrencias` (POST) — `requireRole(ADMIN, FINANCEIRO)`.
...
**UI**: o sino vive no **topbar** (`src/components/topbar/notification-bell.tsx`) entre busca e perfil — popover com últimas 10 não-lidas + link "Ver todas" → `/notificacoes`. Saiu da sidebar (era um item do grupo Configuração). Página `/notificacoes` permanece intacta. `GET /api/notificacoes` devolve **`{ notificacoes }`** (não array cru) e respeita `limit`; contagem em `/api/notificacoes/contar` (`{ total }`). Isolamento por empresa vem da extensão de tenant (`Notificacao` em `TENANT_MODELS`).
